...
// Global variables for M211 counting
volatile uint32_t m211_count = 0;
volatile uint32_t m211_target = 0;
volatile bool m211_done = false;

// =======================
// M211 Falling Edge Counting Logic
// =======================
void m211_start_counter(uint32_t target_count) {
    m211_count = 0;
    m211_target = target_count;
    m211_done = false;

//counting for pin 18 
    
	DDRD &= ~(1 << 3);  // Set pin 18 (PD3) as input
    //EICRA &= ~((1 << ISC30) | (1 << ISC31)); // FALLING edge: 00
    EICRA |= (1 << ISC30) | (1 << ISC31); // Rising edge INT3
...
ISR(INT3_vect) {
    m211_count++;
    //printString("INT5 triggered\r\n");
    if (m211_count >= m211_target) {
        m211_done = true;
        m211_stop_counter();
    }
}
    ...
    while((c = serial_read()) != SERIAL_NO_DATA) {
      if ((c == '\n') || (c == '\r')) { // End of line reached
        ...
        if (line_flags & LINE_FLAG_OVERFLOW) {
          ...
          // === Custom M211 Falling Edge Counting Handler ===
          if (line[0] == 'M' && line[1] == '2' && line[2] == '1' && line[3] == '1') {
            uint32_t target = 0;
            char *ptr = strchr(line, 'P');
            if (ptr != NULL) {
              target = (uint32_t)atol(ptr + 1);
              char debug_buf[32];
              ///sprintf(debug_buf, "Target: %lu\r\n", target);
              //printString(debug_buf);
              //report_feedback_message(MESSAGE_M211_STARTED);
              m211_start_counter(target);
              while (!m211_done) {
                protocol_execute_realtime();
              }
              //report_feedback_message(MESSAGE_M211_DONE);
              report_status_message(STATUS_OK);
            } else {
